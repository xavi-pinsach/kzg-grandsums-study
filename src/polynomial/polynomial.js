    ...
    */
    ...
    static async expX(polynomial, n, truncate = false) {
        ...
        if (n < 1) {
            ...
            throw new Error(
                "Compute a new polynomial to a zero or negative number is not allowed"
            );
        } else if (1 === n) {
            return await Polynomial.fromEvaluations(
                polynomial.coef,
                curve,
                polynomial.logger
            );
    ...
    static computeZHEvaluation(curve, x, nBits) {
        const Fr = curve.Fr;
    ...
    static computeL1Evaluation(curve, x, ZHx, nBits) {
        const Fr = curve.Fr;
